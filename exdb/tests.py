...
from exdb.models import Experience, Type, SubType, Organization, ExperienceComment
...
class ExperienceApprovalViewTest(StandardTestCase):
    ...
    def test_404_when_experience_not_pending(self):
        ...
        client = Client()
        response = client.get(reverse('approval', args=str(e.pk)))
        self.assertEqual(response.status_code, 404, "Attempting to retrieve a non-pending experience did not generate a 404.")

    def test_does_not_allow_deny_without_comment(self):
        e = self.create_experience('pe')
        client = Client()
        response = client.post(reverse('approval', args=str(e.pk)), {'deny': 'deny', 'message': ""})
    ...
    def test_approves_experience_no_comment(self):
        e = self.create_experience('pe')
        client = Client()
        response = client.post(reverse('approval', args=str(e.pk)), {'approve': 'approve', 'message': ""})
        e = Experience.objects.get(pk=e.pk)
    ...
    def test_creates_comment(self):
        ...
        response = client.post(reverse('approval', args=str(e.pk)), {'deny': 'deny', 'message': "Test Comment"})
    ...
    def test_does_not_create_comment(self):
        ...
        response = client.post(reverse('approval', args=str(e.pk)), {'deny': 'deny', 'message': ""})
        comments = ExperienceComment.objects.filter(experience=e)
        self.assertEqual(len(comments), 0, "If message is an empty string, no ExperienceComment object should be created.")
